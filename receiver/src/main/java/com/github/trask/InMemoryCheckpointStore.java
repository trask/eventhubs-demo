...
// COPIED FROM
// https://github.com/Azure/azure-sdk-for-java/blob/master/sdk/eventhubs/azure-messaging-eventhubs/
// src/samples/java/com/azure/messaging/eventhubs/InMemoryCheckpointStore.java

/**
 * A simple in-memory implementation of a {@link CheckpointStore}. This implementation keeps track
 * of partition ownership details including checkpointing information in-memory. Using this
 * implementation will only facilitate checkpointing and load balancing of Event Processors running
 * within this process.
 */
public class InMemoryCheckpointStore implements CheckpointStore {
    ...
    public Flux<PartitionOwnership> listOwnership(String fullyQualifiedNamespace,
        ...
        String prefix =
                prefixBuilder(fullyQualifiedNamespace, eventHubName, consumerGroup, OWNERSHIP);
        return Flux.fromIterable(partitionOwnershipMap.keySet())
                .filter(key -> key.startsWith(prefix))
                .map(key -> partitionOwnershipMap.get(key));
    }

    private String prefixBuilder(String fullyQualifiedNamespace, String eventHubName,
            String consumerGroup,
            String type) {
    ...
    /**
     * Returns a {@link Flux} of partition ownership details for successfully claimed partitions. If
     * a partition is already claimed by an instance or if the ETag in the request doesn't match the
     * previously stored ETag, then ownership claim is denied.
     *
     * @param requestedPartitionOwnerships
     *            List of partition ownerships this instance is requesting to own.
    ...
    public Flux<PartitionOwnership> claimOwnership(
        ...
        String prefix =
                prefixBuilder(firstEntry.getFullyQualifiedNamespace(), firstEntry.getEventHubName(),
        ...
        return Flux.fromIterable(requestedPartitionOwnerships)
                .filter(partitionOwnership -> {
                    return !partitionOwnershipMap.containsKey(partitionOwnership.getPartitionId())
                            || partitionOwnershipMap.get(partitionOwnership.getPartitionId())
                                    .getETag()
                                    .equals(partitionOwnership.getETag());
                })
                .doOnNext(partitionOwnership -> logger
                        .info("Ownership of partition {} claimed by {}",
                ...
                .map(partitionOwnership -> {
                    ...
                    partitionOwnershipMap.put(
                            prefix + SEPARATOR + partitionOwnership.getPartitionId(),
                            partitionOwnership);
    ...
    public Flux<Checkpoint> listCheckpoints(String fullyQualifiedNamespace, String eventHubName,
            String consumerGroup) {
        String prefix =
                prefixBuilder(fullyQualifiedNamespace, eventHubName, consumerGroup, CHECKPOINT);
     ...
     *
     * @param checkpoint
     *            The checkpoint containing the information to be stored in-memory.
    ...
    public Mono<Void> updateCheckpoint(Checkpoint checkpoint) {
        if (checkpoint == null) {
            return Mono.error(logger
                    .logExceptionAsError(new NullPointerException("checkpoint cannot be null")));
        }

        String prefix =
                prefixBuilder(checkpoint.getFullyQualifiedNamespace(), checkpoint.getEventHubName(),
                        checkpoint.getEventHubName(), CHECKPOINT);
        checkpointsMap.put(prefix + SEPARATOR + checkpoint.getPartitionId(), checkpoint);
        logger.info("Updated checkpoint for partition {} with sequence number {}",
